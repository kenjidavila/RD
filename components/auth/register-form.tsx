...
export default function RegisterForm({ onBackToLogin }: RegisterFormProps) {
  ...
  const validateForm = () => {
    ...
    } else if (!/^\d{9,11}$/.test(formData.rnc.trim())) {
      newErrors.rnc = "Formato de RNC inválido";
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        setErrors({ general: result.error || `Error ${response.status}` });
      ...
      <div className="w-full max-w-md">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                className={`w-full px-4 py-3 bg-gray-50 border border-gray-200 text-gray-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 placeholder:text-gray-400 ${errors.rnc ? "border-red-300 focus:ring-red-400" : ""}`}
                required
                disabled={loading}
              />
              {errors.rnc && (
                <p className="text-sm text-red-600 mt-1">{errors.rnc}</p>
              )}
            </div>
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                className={`w-full px-4 py-3 bg-gray-50 border border-gray-200 text-gray-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 placeholder:text-gray-400 ${errors.razonSocial ? "border-red-300 focus:ring-red-400" : ""}`}
                required
                disabled={loading}
              />
              {errors.razonSocial && (
        ...
        <div className="text-center mt-8">
          <p className="text-gray-500 text-sm flex items-center justify-center">
            <Flag className="h-4 w-4 mr-2" />© 2024 e-CF República Dominicana.
